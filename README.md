...
`gparted`  
`resize2fs`  
`apt-get install apt-listbugs apt-listchanges arpwatch auditd bleachbit curl debian-goodies debsecan debsums dnstop firewalk htop iperf ipset ipset-persistent iptables-optimizer iptables-persistent irssi locate lynis lynx macchanger netcat nethogs nload openresolv screen speedtest-cli traceroute ntopng openvpn unbound dnscrypt-proxy isc-dhcp-server hostapd hostapd_cli wpasupplicant wireless-tools iw wvdial`  
